...
// Cover Page 模板
const coverPageTemplate = `# Mutual Non-Disclosure Agreement
...
${'Evaluate the proposed business transaction between the parties.'}
...
${'- [x] Expirs [1 year(s)] from Effective Date.'}
...
// Standard Terms 模板
const standardTermsTemplate = `# Standard Terms

1. **Introduction**. This Mutual Non-Disclosure Agreement (which incorporates these Standard Termsand the Cover Page (defined below))("MNDA")allows eachparty("Disclosing Party") to disclose or make available information in connection with the Purpose which (1) the Disclosing Party identifies to the receiving party("Receiving Party") asconfidential",proprietary",or the like or(2)should be reasonably understood as confidential or proprietary due to its nature and the circumstances of its disclosure("Confidential Information").Each party's Confidential Information also includes the existence and status of the parties'sections and information on the Cover Page. Confidential Information includes technical or business information,product designs orroadmaps,requirements,pricing,security and compliance documentation,technology,inventions and know-how. To use this MNDA,the parties must complete and signa cover page incorporating these Standard Terms(Cover Page).Each party is identified on the Cover Page and capitalized terms have the meanings given herein or on the Cover Page.
...
export function generateNDADocument(data: NDAPayload): string {
  // 替换 Cover Page 模板
  const coverPage = replacePlaceholders(coverPageTemplate, data);

  // 替换 Standard Terms 模板
  const standardTerms = replacePlaceholders(standardTermsTemplate, data);

  // 生成 Party 信息表
  const partyTable = `|| PARTY 1|PARTY 2|
|:---|:----:|:----:|
|Signature|${data.party1Signature}|${data.party2Signature}|
...
|Notice Address (Use email or postal address)|${data.party1Address}|${data.party2Address}|
